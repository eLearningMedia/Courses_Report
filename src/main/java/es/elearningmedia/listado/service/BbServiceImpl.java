...
import es.elearningmedia.listado.repository.BbRepository;
...
public class BbServiceImpl implements BbService {
	...
	public List<Alumno> getEnroledStudents(String courseId) {
		...
		try {
			...
			resultado = bbRepository.getFotos(ids);
			
			//Order students by lastname
			Map<String,Alumno> sorter = new TreeMap<String,Alumno>();
			for(Alumno element : resultado) {
				sorter.put(element.getLastname() + element.getFirstname() + element.getMiddlename() + element.getEmail(), element);
			}
			
			resultado = new ArrayList<Alumno>(sorter.values());
	...
	public StringsBundle getBundle() {
		...
		PlugIn plugin = PlugInManagerFactory.getInstance().getPlugIn("elea",
				"listadodealumnos");
		BbResourceBundle resourceBundle = BundleManagerFactory.getInstance()
				.getPluginBundle(plugin.getId());
